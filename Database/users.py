...
def add_user(surname: str, first_name: str, username: str, email: str, password: str):
    """
    ...
    with get_database_connection() as connection:
        with connection.cursor(dictionary=True) as cursor:
            ...
            cursor.execute("""INSERT
                                INTO users
                                   (surname, first_name, username, email, password) 
                              VALUES (%s, %s, %s, %s, %s)""", [surname, first_name, username, email, hashlib.sha256(password_bytes).hexdigest()])
            connection.commit()
...
def email_available(email):
    ...
    with get_database_connection() as connection:
        with connection.cursor(dictionary=True) as cursor:
            cursor.execute("""SELECT u.user_id, u.first_name, u.email
                                FROM users AS u
